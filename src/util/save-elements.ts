...
export async function saveHtml(input: MboxMessage, dir: string): Promise<void> {
  ...
  return jetpack.writeAsync(filename, input.html)
    .then(() => { if (input.date) utimes(filename, input.date, input.date) });
}
...
export async function saveHeaders(input: MboxMessage, output: string): Promise<void> {
  ...
  return jetpack.writeAsync(filename, JSON.stringify(input.headers))
    .then(() => { if (input.date) utimes(filename, input.date, input.date) });
}
...
export async function saveAttachments(input: MboxMessage, dir: string) {
  ...
  for (const attachment of input.attachments) {
    ...
    promises.push(
      attachments.writeAsync(filename, attachment.content)
        .then(() => { if (input.date) utimes(attachments.path(filename), input.date, input.date) })
    );
  }
